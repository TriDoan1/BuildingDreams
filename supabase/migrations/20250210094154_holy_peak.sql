...
INSERT INTO socal_cities (city_name, county, state_code, latitude, longitude, population) VALUES
  ...
  ('Monterey Park', 'Los Angeles', 'CA', 34.0625, -118.1228, 60269),
  ('Gardena', 'Los Angeles', 'CA', 33.8883, -118.3089, 58829),
  ('Diamond Bar', 'Los Angeles', 'CA', 34.0286, -117.8103, 55720),
  ('Arcadia', 'Los Angeles', 'CA', 34.1397, -118.0353, 56364),
  ('Cerritos', 'Los Angeles', 'CA', 33.8583, -118.0647, 49578),
  ...
  ('Seal Beach', 'Orange', 'CA', 33.7414, -118.1048, 24168),
  ('Laguna Hills', 'Orange', 'CA', 33.5963, -117.7178, 31508),
  ('San Juan Capistrano', 'Orange', 'CA', 33.5017, -117.6625, 35911),
  ('La Palma', 'Orange', 'CA', 33.8464, -118.0467, 15568),
  ...
  ('Indian Wells', 'Riverside', 'CA', 33.7175, -116.3417, 5428),
  ('Eastvale', 'Riverside', 'CA', 33.9525, -117.5847, 69757),
  ('Blythe', 'Riverside', 'CA', 33.6178, -114.5882, 19255),
  ('Desert Edge', 'Riverside', 'CA', 33.9372, -116.4097, 3822),
  ('East Hemet', 'Riverside', 'CA', 33.7400, -116.9114, 17418),

  -- Inland Empire (San Bernardino County) (30 cities)
  ...
  ('Muscoy', 'San Bernardino', 'CA', 34.1503, -117.3484, 10644),
  ('Mentone', 'San Bernardino', 'CA', 34.0706, -117.1347, 9000),
  ('Phelan', 'San Bernardino', 'CA', 34.4264, -117.5717, 14304),
  ('Running Springs', 'San Bernardino', 'CA', 34.2078, -117.1092, 4862),
  ('Lake Arrowhead', 'San Bernardino', 'CA', 34.2483, -117.1897, 12424),
  ('Joshua Tree', 'San Bernardino', 'CA', 34.1347, -116.3131, 7414);
...
-- Create array of construction specialties
DO $$
DECLARE
  ...
  first_names text[] := ARRAY[
    ...
    'Lisa', 'Robert', 'Jennifer', 'William', 'Emma',
    'John', 'Elizabeth', 'Richard', 'Linda', 'Thomas',
    'Patricia', 'Charles', 'Barbara', 'Joseph', 'Susan',
    'Christopher', 'Margaret', 'Daniel', 'Sandra', 'Matthew'
  ];
  last_names text[] := ARRAY[
    'Smith', 'Garcia', 'Chen', 'Johnson', 'Kim',
    'Brown', 'Martinez', 'Wilson', 'Anderson', 'Taylor',
    'Thomas', 'Lee', 'Rodriguez', 'Lopez', 'Nguyen',
    'White', 'Williams', 'Davis', 'Miller', 'Jones',
    'Patel', 'Jackson', 'Martin', 'Thompson', 'Moore'
  ];
  roles text[] := ARRAY[
    'Architect', 'Interior Designer', 'General Contractor',
    'Soil Engineer', 'Civil Engineer', 'Structural Engineer',
    'Concrete Specialist', 'Steel Fabricator', 'Framing Contractor',
    'Roofing Contractor', 'Master Plumber', 'Master Electrician',
    'Solar Installer', 'HVAC Technician', 'Cabinet Maker',
    'Window Installer', 'Stucco Specialist', 'Mason',
    'Landscape Architect', 'Pool Contractor', 'Glazier',
    'Drywall Contractor', 'Finish Carpenter', 'Tile Setter',
    'Smart Home Integrator', 'Painter', 'Flooring Specialist',
    'Security Systems Expert', 'Lighting Designer', 'Home Automation Specialist'
  ];
...
BEGIN
  ...
  FOR counter IN 1..100 LOOP
    ...
    VALUES (
      city_record.county || ' ' || specialties[1 + (counter % array_length(specialties, 1))] || ' Specialists',
  ...
  FOR counter IN 1..500 LOOP
    ...
    FOR i IN 1..3 LOOP
      IF random() < 0.3 THEN  -- 30% chance for each specialty
        selected_specialty := specialties[1 + (random() * (array_length(specialties, 1) - 1))::integer];
        
        -- Only insert if this specialty doesn't exist for this professional
        IF NOT EXISTS (
          SELECT 1 FROM professional_specialties
          WHERE professional_id = prof_id AND specialty = selected_specialty
        ) THEN
          INSERT INTO professional_specialties (professional_id, specialty)
          VALUES (prof_id, selected_specialty);
        END IF;
      END IF;
    END LOOP;

    -- Add 1-3 certifications
    FOR i IN 1..3 LOOP
      IF random() < 0.3 THEN  -- 30% chance for each certification
        selected_certification := 'Certified ' || specialties[1 + (random() * (array_length(specialties, 1) - 1))::integer] || ' Professional';
        
        -- Only insert if this certification doesn't exist for this professional
        IF NOT EXISTS (
          SELECT 1 FROM professional_certifications
          WHERE professional_id = prof_id AND certification = selected_certification
        ) THEN
  ...
  -- Create 75 projects
  FOR counter IN 1..75 LOOP
    -- Get a random architect, designer, or contractor
    SELECT id, role INTO prof_id, prof_role
    FROM professionals
    WHERE role IN ('Architect', 'Interior Designer', 'General Contractor')
    ORDER BY random()
    LIMIT 1;
    ...
    VALUES (
      ...
      'Professional ' || prof_role || ' project in ' || city_record.city_name || ' featuring custom design and premium materials.',
      'https://images.unsplash.com/photo-' || (
        CASE (counter % 5)
